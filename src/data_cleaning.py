...
def clean_wallace_data(data: pd.DataFrame) -> pd.DataFrame:
    ...
    if 'days_since_last_contact_previous_campaign' in clean_data.columns:
        ...
        clean_data['days_since_last_contact_previous_campaign'] = clean_data[
            'days_since_last_contact_previous_campaign'].replace(
                -1, fill_value)

    # -- Map Target to 0/1 --
    if 'new_contract_this_campaign' in clean_data.columns:
